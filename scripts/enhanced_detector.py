    ...
    def __init__(self, cfg: Dict):
        ...
        if self.use_cava:
            self.cava = CAVAModule(
                ...
                gate_clip_min=float(cava_cfg.get('gate_clip_min', cava_cfg.get('gate_min', 0.05))),
                gate_clip_max=float(cava_cfg.get('gate_clip_max', cava_cfg.get('gate_max', 0.95))),
                ...
                window_size=int(cava_cfg.get('window_size', 5)),
                mask_type=str(cava_cfg.get('mask_type', 'hard')),
                multi_scale=bool(cava_cfg.get('multi_scale', False)),
                gate_range_mode=str(cava_cfg.get('gate_range_mode', 'paper')),
                gate_range=cava_cfg.get('gate_range', None),
            )
        else:
            self.cava = None
        ...
        self.mil_head = EnhancedMILHead(
            ...
            dropout=float(mil_cfg.get('dropout')),
            topk_ratio=float(mil_cfg.get('topk_ratio')),
            attn_temp=float(mil_cfg.get('attn_temp')),
        )
        # ★ 推荐 alias：让主路径也能命中
    ...
    def forward(self, video, audio, return_aux: bool = True):
        ...
        if use_cava_flag:
            ...
            cava_aux = {
                ...
                "delta_low":        delta_low_v,
                "delta_high":       delta_high_v,
