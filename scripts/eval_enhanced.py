#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Minimal enhanced evaluator entrypoint (Phase 4 compatibility)."""
import argparse
import json
...
def evaluate(cfg: dict, checkpoint: str, out_dir: str, device_mode: str = "auto", max_batches: int = 0):
    ...
    model = EnhancedAVTopDetector(
        {
            "model": model_cfg,
            "fusion": model_cfg.get("fusion", c.get("fusion", {})),
            "cava": c.get("cava", {}),
            "data": data_cfg,
            "video": c.get("video", {}),
            "audio": c.get("audio", {}),
    ...
    loader = DataLoader(
        ds,
        batch_size=int(c.get("training", {}).get("batch_size", 16)),
        shuffle=False,
        num_workers=int(data_cfg.get("num_workers_val", 2)),
