...
def main():
    ...
    ap.add_argument("--fps", type=float, default=30.0, help="视频 FPS")
    ap.add_argument("--out", type=str, default="physical_prior.yaml", help="输出 YAML 文件路径")
    ap.add_argument("--delta_low_frames", type=int, default=-10, help="paper prior low bound (frames)")
    ap.add_argument("--delta_high_frames", type=int, default=150, help="paper prior high bound (frames)")
    args = ap.parse_args()
    ...
    # Paper-first prior range
    low_frames = int(args.delta_low_frames)
    high_frames = int(args.delta_high_frames)
